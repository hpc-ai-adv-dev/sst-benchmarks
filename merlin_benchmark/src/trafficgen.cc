...
using namespace SST::MerlinBenchmark;
...
//    last_vc(0),
    ...
    packetDelayGen(NULL)
{
    auto output_file_name = params.find<std::string>("output_file_name", "");
    if ( !out_initialized ) {
        if (output_file_name.empty()) {
            out.init("CHECK @t: ", getSimulationOutput().getVerboseLevel(), Output::PrintAll, Output::STDOUT);
        } else {
            out.init("CHECK @t: ", getSimulationOutput().getVerboseLevel(), Output::PrintAll, Output::FILE, output_file_name);
        }
        out_initialized = true;
    }

    out.verbose(CALL_INFO, 5, 0, "Creating TrafficGen with id=%d, num_peers=%d\n", params.find<int>("id",-1), params.find<int>("num_peers",-1));
    out.flush();
